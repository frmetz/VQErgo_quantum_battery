...
dir_name = f"results/n{n}_J{J}_h{h}/m{m}/"
...
for i,index_rep in enumerate(index_ibm):
    ...
    save_name_quantities_ibm = dir_name_quantities_ibm + f"quantities_seed{34}"
...
ax1.errorbar(times, ergo_average, yerr=standard_deviation_ergo,capsize =6 ,ms=8, marker = 'o',ls='',label=r"$\mathcal{E}$-FakePerth",color='dodgerblue',zorder=2)
ax1.plot(times_ibm,ergo_list_ibm,ms=10, marker = 'h',ls='',label=r"$\mathcal{E}$-IBMPerth",color=colors[3],zorder=3)
# fig.suptitle(r"$N=%s,M=%s,h=%s,J=%s$" %(n,m,-h,-J))
...
order = [0,2,1,3,4,5]
ax1.legend([handles[idx] for idx in order],[labels[idx] for idx in order],loc=8) 
ax1.tick_params('both', length=10, width=2, which='major')
plt.tight_layout()
plt.savefig(dir_name_ibm+f"pvqd_runtime_N{n}_M{m}_ver2.eps", dpi=300)
plt.show()
