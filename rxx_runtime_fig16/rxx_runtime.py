...
np.random.seed(seed)
dir_name = "results/"
dir_name = dir_name + f"n{n}_J{J}_h{h}/time{max_time}_reps{reps}/"
dir_name = dir_name + f"m{m}/"
os.makedirs(dir_name, exist_ok=True)

##########################################################################################################################################################

service = QiskitRuntimeService(channel="ibm_quantum",instance='...',token="...")


#backend = service.get_backend("ibm_perth")
backend = service.get_backend("ibmq_qasm_simulator")
options = Options(resilience_level = 1, execution=ExecutionOptions(shots=2048), max_execution_time=10500)
############################################################# Prepare Hamiltonian ########################################################################
...
l = 7 #the index of time that we want to extract information
...
with Session(service=service, backend=backend) as session:
    ...
    optimizer_passive    = SPSA(maxiter=250)
    start_time           = timeit.default_timer()
    vqe                  = VQE(estimator,ansatz=full_circuit, 
                            optimizer=optimizer_passive,
